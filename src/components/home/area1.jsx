...
export const Area1 = () => {
  ...
  useEffect(() => {
    const update = () => {
      if (bannerRef.current && h1Ref.current) {
        const bannerBottom = bannerRef.current.getBoundingClientRect().bottom;
        const h1Top = h1Ref.current.getBoundingClientRect().top;
        setClipHeight(Math.max(0, bannerBottom - h1Top));
      }
    };
    update();
  ...
  const buttonp = "text-p text-white font-medium self-center ml-6 max-[480px]:ml-0";
    ...
    <div className="h-screen [@media(min-height:1312px)]:h-[1080px] [@media(min-height:1312px)]:mb-33 min-[1920px]:w-[1920px] min-[1920px]:justify-self-center">
      <div ref={bannerRef} className="relative h-[60vh] max-h-[768px]">
        ...
        <div className="absolute top-full -translate-y-[30%] [@media(min-height:800px)]:-translate-y-[24%] justify-self-center w-[calc(100vw-160px)] max-w-[calc(1920px-160px)] max-[768px]:w-[calc(100vw-112px)] max-[480px]:w-[calc(100vw-64px)]">
          ...
          <div ref={h1Ref} className="relative mb-3">
            <h1 className="[grid-area:area1h1] text-h1 font-medium max-[1024px]:text-h1-2 max-[768px]:text-h1-3 max-[640px]:text-h2 max-[320px]:text-h2-2 max-[640px]:w-[346px] max-[480px]:w-[216px] text-black">
              Rap Scannr
            </h1>
            <h1
              className="[grid-area:area1h1] text-h1 font-medium max-[1024px]:text-h1-2 max-[768px]:text-h1-3 max-[640px]:text-h2 max-[320px]:text-h2-2 max-[640px]:w-[346px] max-[480px]:w-[216px] text-white absolute top-0 left-0 overflow-hidden pointer-events-none"
              style={{ clipPath: `inset(0 0 calc(100% - ${clipHeight}px) 0)` }}
            >
              Rap Scannr
            </h1>
          </div>
