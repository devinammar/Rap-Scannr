...
// ini pake youtube API dari RapidAPI (versi fix issue)
...
const downloadYoutubeAudio = async (url) => {
  ...
  if (!mp3Url) throw new Error("Failed to get MP3 URL from RapidAPI");

  const username = process.env.RAPIDAPI_USERNAME;
  const xRun = crypto.createHash("md5").update(username).digest("hex");
  ...
  const mp3Response = await axios.get(mp3Url, {
    ...
    headers: {
      "User-Agent": `Mozilla/5.0 ${username}`,
      "x-run": xRun,
    },
  });
